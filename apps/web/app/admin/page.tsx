...
import { ArrowRight, BadgePercent, FolderTree, RotateCcw, ShieldCheck, Store, UserCog, UsersRound } from 'lucide-react';
import Link from 'next/link';

const ADMIN_OPERATIONS = [
  { href: '/admin/users', title: 'Quản lý người dùng', description: 'Tra cứu, kiểm tra và khóa hoặc mở khóa tài khoản.', icon: UsersRound, tone: 'bg-sky-100 text-sky-700' },
  { href: '/admin/shops', title: 'Quản lý cửa hàng', description: 'Duyệt hồ sơ, đình chỉ và khôi phục hoạt động shop.', icon: Store, tone: 'bg-blue-100 text-blue-700' },
  ...
  return (
    <AppShell>
      <section className="overflow-hidden rounded-3xl border border-[#cfe3da] bg-[#e5f3ed] px-6 py-8 sm:px-9">
        <div className="flex flex-col justify-between gap-7 md:flex-row md:items-center">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-xl text-sm leading-6 text-[#4d6a60]">Workspace dành riêng cho Admin để quản trị người dùng, cửa hàng, taxonomy, coupon và luồng hoàn tiền.</p>
          </div>
      ...
      <section className="mt-7 grid gap-4 lg:grid-cols-3">
        <div className="surface-card p-6 lg:col-span-2"><p className="eyebrow">Thứ tự ưu tiên</p><h2 className="mt-2 text-xl font-black">Checklist vận hành đề xuất</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{['Rà soát tài khoản bất thường', 'Xử lý hồ sơ cửa hàng đang chờ', 'Kiểm tra danh mục trước khi khóa', 'Đối soát coupon và refund'].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-xl bg-[#f5f8f6] p-3.5"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-xs font-black text-[var(--accent)] shadow-sm">{index + 1}</span><span className="text-sm font-bold">{item}</span></div>)}</div></div>
